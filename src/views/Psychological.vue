        ...
        <v-row align="center" justify="center">
                ...
                <v-card>
                    ...
                    <v-card-text>
                        <v-list two-line>
                            ...
                            <v-list-item v-for="[i, contact] in contacts.entries()" :key="i" :href="`tel://${contact.phone}`">
                                <v-list-item-avatar>
                                    <v-icon>
                                        mdi-contact-phone
                                    </v-icon>
                    ...
                    <v-card-actions>
                        <v-spacer />
                        <v-btn text @click="dialog.enabled = false">
                            关闭
                        </v-btn>
                    </v-card-actions>
                </v-card>
            ...
            <v-dialog v-model="report.enabled" persistent max-width="450px">
                <v-card>
                    ...
                    <v-card-text>
                        <v-select v-model="report.cause" label="错误类型" placeholder="请选择错误类型" :items="report.causes" filled hide-details class="mb-2" />
                        收到纠错请求后我们会再次审核此条信息以保证准确性。<br>
                        <span class="red--text">注意：我们的人力资源有限，烦请不要滥用此功能，十分感谢！</span>
                    </v-card-text>
                    <v-card-actions>
                        <v-btn text @click="report.enabled = false">
                            取消
                        </v-btn>

                        <v-spacer />

                        <v-btn text color="primary" :loading="$store.getters.ajaxLoading" :disabled="report.cause === ''" @click="doReport">
                            确认
                        </v-btn>
                    </v-card-actions>
                </v-card>
            ...
            <v-col cols="12">
                ...
                <p class="subtitle-1 mx-3">
                    图例：<v-icon small>mdi-bed-empty</v-icon>表示床位剩余，<v-icon small>mdi-home-circle</v-icon>表示房间剩余
                </p>
                <v-skeleton-loader :loading="$store.getters.ajaxLoading && !data.length" type="card@4" class="mx-3">
                    <div>
                        <v-text-field v-model="search" label="搜索" placeholder="心理咨询机构名称、地址或区域" outlined hide-details clearable />
                        <v-data-iterator :items="dataset" :search="search" :page="page" :items-per-page="20" hide-default-footer>
                            ...
                            <template v-slot:header="{ pagination }">
                                <Paginator :page="page" :pagination="pagination" @change="(newPage) => {page = newPage}" />
                            </template>
                            <template v-slot:default="{ items }">
                                <v-card v-for="[i, o] in items.entries()" :key="i" class="my-4 pb-2 card-border">
                                    <v-card-title>
                                    ...
                                    <v-card-text>
                                        <span class="float-right ml-4">
                                            <v-icon class="mr-1" small>mdi-bed-empty</v-icon> {{ o.beds ? o.beds : "未知" }}
                                            <br>
                                            <v-icon class="mr-1" small>mdi-home-circle</v-icon> {{ o.room ? o.room : "未知" }}
                                        </span>
                                        <span class="subtitle-1">
                                            {{ o.province }} {{ o.city }} {{ o.suburb }}<br>地址：{{ o.address }}
                                        </span>
                                        <br>
                                        <span v-if="o.notes.length" class="subtitle-2 red--text">
                                            备注：{{ o.notes }}
                                        </span>
                                    </v-card-text>
                                    <v-card-actions class="mx-2">
                                        <v-btn outlined color="primary" :href="`https://ditu.amap.com/search?query=${encodeURIComponent(o.name)}`" target="_blank">
                                            <v-icon left>
                                                mdi-map-marker
                                            </v-icon>
                                            地图
                                        </v-btn>
                                        <v-btn outlined color="green" @click="openDialog(o)">
                                            <v-icon left>
                                                mdi-contact-phone
                                            </v-icon>
                                            联系方式
                                        ...
                                        <v-spacer />
                                        <v-btn icon color="error darken-1" @click="openReport(o)">
                                            <v-icon>
                                                mdi-flag
                                            </v-icon>
                                        </v-btn>
                            ...
                            <template v-slot:footer="{ pagination }">
                                <Paginator :page="page" :pagination="pagination" @change="(newPage) => {page = newPage; $vuetify.goTo(0, {duration: 225, offset: 0, easing: 'easeOutQuad'})}" />
                            </template>
                        </v-data-iterator>
                    </div>
                </v-skeleton-loader>
